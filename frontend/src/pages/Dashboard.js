...
const DoctorDashboard = () => {
  ...
  const loadPatients = async () => {
    ...
    console.log('🔍 DoctorDashboard loadPatients - Token present:', !!token);
    
    if (!token) {
      console.warn('⚠️ No token available, skipping patient load');
      return;
    }
...
const DepartmentDashboard = () => {
  ...
  const loadActions = async () => {
    const token = localStorage.getItem('token');
    console.log('🔍 DepartmentDashboard loadActions - Token present:', !!token);
    
    if (!token) {
      console.warn('⚠️ No token available, skipping actions load');
      return;
    }
