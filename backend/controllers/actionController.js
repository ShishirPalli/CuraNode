...
exports.updateActionStatus = async (req, res) => {
  try {
    ...
    // Get the action to check department assignment
    const action = await ClinicalAction.findById(req.params.id);
    if (!action) {
      return res.status(404).json({ message: 'Action not found' });
    }

    // Map department to role(s) that can update it
    const canUpdateByRole = {
      'pharmacy': ['pharmacy'],
      'lab': ['diagnostic-staff'],
      'imaging': ['diagnostic-staff'],
      'nursing': ['nurse'],
    };

    // Check if user's role is allowed to update this action
    const allowedRoles = canUpdateByRole[action.departmentAssigned] || [];
    if (!allowedRoles.includes(req.user.role)) {
      return res.status(403).json({ 
        message: `Only ${allowedRoles.join(' or ')} can update this action status` 
      });
    }
    ...
    const updatedAction = await ClinicalAction.findByIdAndUpdate(
      req.params.id,
      updateData,
      { new: true }
    )
    ...
    // Emit socket event for real-time update
    if (req.socketManager) {
      req.socketManager.emitActionUpdate(updatedAction.patientId._id.toString(), updatedAction);
      req.socketManager.emitActionToRole(req.user.role, updatedAction);
    }
